...
public class TimedSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        if (currentTime > 0)
        {
            currentTime -= Time.deltaTime;//Make sure you're letting timer count down to zero instead of making the spawn interval approach negative infinity (Cal was here)
        }
        else
        {
            Spawn();
